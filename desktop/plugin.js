/**
 * Ollama Cloud Usage — desktop plugin (statusbar chip + popover).
 *
 * Reads the plugin's own backend namespace via `ctx.rest('/usage')`, which the
 * SDK routes to `/api/plugins/ollama-cloud-usage/usage` (the FastAPI router in
 * dashboard/plugin_api.py). That backend proxies the official
 * ollama.com/api/usage endpoint — no cookie scraping.
 *
 * Ollama now bills usage as a MONTHLY usage-credit pool: a single
 * `limits.monthly` fraction + per-model request counts. The chip shows
 * `M:<pct>`; the popover a single ring + "This month" model breakdown.
 *
 * Ships OFF by default (defaultEnabled: false).
 *
 * Plain ESM, loaded uncompiled — UI is jsx() calls, not JSX syntax.
 * Only these imports resolve: the plugin SDK, react, react/jsx-runtime.
 *
 * IMPORTANT: layout/sizing/colors use INLINE STYLES, not Tailwind classes.
 * The app's CSS is purged at build time and only contains classes the app
 * itself uses, so a plugin class the app never references is absent at runtime
 * (collapsing the layout). Inline styles resolve regardless. Theme CSS vars
 * (var(--ui-*)) DO resolve at runtime, so colors reference them directly.
 */

import {
  Codicon,
  haptic,
  PALETTE_AREA,
  queryClient,
  useQuery,
  useQueryClient
} from '@hermes/plugin-sdk'
import { jsx, jsxs } from 'react/jsx-runtime'

const ID = 'ollama-cloud-usage'
const USAGE_KEY = ['ollama-cloud-usage', 'usage']

// Bound in register(); the pane/chip read it through React Query's queryFn.
let rest = null

function fetchUsage() {
  if (!rest) return Promise.reject(new Error('plugin backend not bound'))
  return rest('/usage')
}

// Theme tokens (all verified to exist in the app's compiled CSS).
const C = {
  primary: 'var(--ui-text-primary)',
  secondary: 'var(--ui-text-secondary)',
  tertiary: 'var(--ui-text-tertiary)',
  quaternary: 'var(--ui-text-quaternary)',
  track: 'var(--ui-bg-quaternary)',
  accent: 'var(--ui-accent)',
  warn: 'var(--ui-yellow)',
  bad: 'var(--ui-red)',
  stroke: 'var(--ui-stroke-secondary)'
}

// ---------------------------------------------------------------------------
// Ollama logo (official llama head, docs/logo.svg) — currentColor.
// ---------------------------------------------------------------------------

const OLLAMA_LOGO_PATH =
  'M7.25558 0.114339C7.61134 0.222519 7.93252 0.400698 8.22405 0.636149C8.70993 1.0256 9.12005 1.58303 9.433 2.24356C9.74758 2.90792 9.95182 3.64354 10.0292 4.38171C11.0662 3.9284 12.2171 3.65235 13.4041 3.57227L13.4881 3.56718C14.921 3.47809 16.3375 3.6779 17.5728 4.17044C17.7391 4.2379 17.9022 4.31044 18.062 4.3868C18.1443 3.66263 18.3453 2.94355 18.6549 2.29447C18.9678 1.63266 19.378 1.07651 19.8622 0.685785C20.1328 0.459579 20.4638 0.281532 20.8323 0.163974C21.2556 0.0367035 21.7053 0.0137947 22.1434 0.110521C22.8039 0.255609 23.3704 0.578877 23.8168 1.04851C24.2253 1.47739 24.5316 2.0272 24.7408 2.68646C25.1196 3.87517 25.1855 5.43933 24.9302 7.32549L25.0175 7.37639L25.0603 7.40058C26.3072 8.13366 27.1752 9.17855 27.6348 10.3914C28.3512 12.284 27.9905 14.4068 26.7552 15.5943L26.7255 15.621L26.7288 15.6248C27.4157 16.5946 27.8324 17.6192 27.9214 18.6793L27.9246 18.7175C28.0301 20.0729 27.5952 21.4373 26.5839 22.7774L26.5723 22.7902L26.5888 22.8207C27.3663 24.2932 27.6101 25.7759 27.3103 27.2574L27.3004 27.307C27.254 27.5234 27.0983 27.7168 26.8677 27.8446C26.637 27.9724 26.3501 28.0246 26.07 27.9892C25.9312 27.9724 25.7982 27.9347 25.6783 27.8782C25.5585 27.8217 25.4543 27.7474 25.3717 27.6595C25.289 27.572 25.2296 27.4725 25.1968 27.3668C25.164 27.2614 25.1585 27.152 25.1806 27.0448C25.4556 25.7301 25.197 24.4116 24.39 23.0702C24.3147 22.9456 24.2812 22.8083 24.2927 22.671C24.3043 22.5338 24.3604 22.401 24.4559 22.2849L24.4624 22.2773C25.4573 21.1013 25.869 19.9482 25.7801 18.8155C25.7043 17.8241 25.2448 16.8504 24.4624 15.9226C24.3103 15.7423 24.2561 15.5229 24.3115 15.3119C24.367 15.1009 24.5277 14.9152 24.7589 14.795L24.7737 14.7874C25.174 14.585 25.5429 14.0683 25.729 13.3619C25.9344 12.5267 25.8808 11.6658 25.5726 10.8496C25.2349 9.95872 24.6173 9.21546 23.7526 8.70765C22.7726 8.12984 21.4747 7.85111 19.8326 7.9313C19.6178 7.94209 19.4039 7.90286 19.2183 7.81869C19.0327 7.73451 18.8841 7.60927 18.7916 7.45912C18.2744 6.61277 17.5201 6.00696 16.5796 5.63151C15.6767 5.2833 14.6658 5.13696 13.661 5.20897C11.6104 5.33497 9.80194 6.22841 9.26335 7.35476C9.18715 7.51329 9.05009 7.65005 8.87052 7.74673C8.69096 7.84338 8.47747 7.89535 8.25864 7.89566C6.50122 7.8982 5.14075 8.21638 4.14592 8.79037C3.28615 9.28673 2.6998 9.98036 2.39015 10.8114C2.10995 11.5937 2.07158 12.4159 2.27815 13.2118C2.46262 13.9219 2.82333 14.5099 3.23674 14.8268L3.24992 14.8357C3.5991 15.0992 3.67321 15.5103 3.42945 15.8348C2.83651 16.6264 2.39345 17.8062 2.32098 18.9402C2.23862 20.2358 2.62733 21.3609 3.50521 22.1678L3.53157 22.192C3.66406 22.3113 3.74924 22.4576 3.77701 22.6133C3.80475 22.769 3.77385 22.9276 3.68804 23.0702C2.73933 24.6432 2.4478 25.9363 2.76239 26.9545C2.81892 27.1662 2.76631 27.3867 2.61573 27.5687C2.46516 27.7509 2.22851 27.8805 1.95615 27.9299C1.68379 27.9795 1.39724 27.9446 1.15746 27.8334C0.917644 27.7219 0.743586 27.5427 0.672268 27.3337C0.272031 26.0381 0.543797 24.5541 1.45133 22.8818L1.47438 22.8373L1.46121 22.822C1.01515 22.3129 0.682282 21.7498 0.476267 21.156L0.468032 21.1318C0.218008 20.391 0.119645 19.6244 0.176502 18.86C0.248972 17.7019 0.634385 16.5157 1.20097 15.5637L1.22074 15.5306L1.21744 15.5281C0.734856 14.9961 0.377443 14.3152 0.179796 13.5618L0.17156 13.5312C-0.100765 12.4803 -0.0482896 11.3945 0.324737 10.3622C0.756268 9.19764 1.6045 8.19729 2.85462 7.47439C2.95345 7.41712 3.05721 7.35985 3.16098 7.3064C2.89909 5.40624 2.96498 3.8319 3.34545 2.63556C3.55463 1.97629 3.86263 1.42648 4.2711 0.997598C4.71581 0.529242 5.2824 0.205974 5.94287 0.0596123C6.38099 -0.0371136 6.83228 -0.0142049 7.25558 0.114339ZM14.0349 11.6832C15.5765 11.6832 16.9996 12.0816 18.0636 12.7714C19.1013 13.4421 19.7189 14.3432 19.7189 15.2405C19.7189 16.3706 19.0502 17.2513 17.8528 17.8139C16.8316 18.2911 15.4629 18.5228 13.8949 18.5228C12.233 18.5228 10.8132 18.1931 9.78876 17.5886C8.77252 16.9904 8.20264 16.1504 8.20264 15.2405C8.20264 14.3407 8.85817 13.437 9.94194 12.7638C11.0422 12.0803 12.4949 11.6832 14.0349 11.6832ZM14.0349 12.8236C12.8922 12.8159 11.7798 13.1075 10.8791 13.6508C10.1198 14.1217 9.68994 14.7136 9.68994 15.2417C9.68994 15.7865 10.0358 16.2968 10.6946 16.685C11.4441 17.1266 12.5459 17.3824 13.8949 17.3824C15.2109 17.3824 16.321 17.1953 17.077 16.8403C17.8396 16.4839 18.23 15.9672 18.23 15.2405C18.23 14.7021 17.8248 14.1077 17.105 13.6419C16.3078 13.1265 15.2274 12.8236 14.0349 12.8236ZM15.1252 14.3636L15.1318 14.3687C15.3295 14.5608 15.2883 14.8396 15.0396 14.9923L14.5587 15.285V15.8526C14.5578 15.979 14.4921 16.0999 14.376 16.1889C14.2599 16.2779 14.1029 16.3277 13.9394 16.3274C13.7758 16.3277 13.6188 16.2779 13.5027 16.1889C13.3866 16.0999 13.3209 15.979 13.3201 15.8526V15.2672L12.8737 14.9897C12.8148 14.9533 12.7659 14.9082 12.7297 14.857C12.6935 14.8059 12.6707 14.7497 12.6628 14.6917C12.6548 14.6337 12.6618 14.5751 12.6833 14.5192C12.7048 14.4633 12.7404 14.4113 12.7881 14.3661C12.8853 14.2747 13.0253 14.2166 13.1776 14.2044C13.3299 14.1923 13.4824 14.2271 13.6017 14.3012L13.9558 14.5201L14.3182 14.2987C14.4371 14.2261 14.588 14.1922 14.7388 14.2043C14.8896 14.2165 15.0282 14.2736 15.1252 14.3636ZM6.82405 11.9212C7.61134 11.9212 8.25205 12.4176 8.25205 13.0298C8.25248 13.3232 8.10217 13.6048 7.83409 13.8127C7.56602 14.0205 7.20215 14.1376 6.8224 14.1383C6.44321 14.1373 6.08 14.0202 5.81235 13.8127C5.54467 13.6051 5.3944 13.324 5.3944 13.031C5.39351 12.7376 5.54342 12.4559 5.81117 12.2478C6.07895 12.0397 6.4443 11.9223 6.82405 11.9212ZM21.1634 11.9212C21.954 11.9212 22.593 12.4176 22.593 13.0298C22.5935 13.3232 22.4432 13.6048 22.1751 13.8127C21.907 14.0205 21.5431 14.1376 21.1634 14.1383C20.7842 14.1373 20.421 14.0202 20.1533 13.8127C19.8857 13.6051 19.7354 13.324 19.7354 13.031C19.7345 12.7376 19.8844 12.4559 20.1522 12.2478C20.4199 12.0397 20.7836 11.9223 21.1634 11.9212ZM6.48969 1.6543L6.48475 1.65684C6.29392 1.72096 6.131 1.82611 6.01534 1.95975L6.0071 1.96738C5.77981 2.20793 5.58216 2.56174 5.43393 3.02628C5.15392 3.90699 5.07816 5.10206 5.22969 6.56695C5.93793 6.40405 6.7104 6.30223 7.54217 6.26532L7.55864 6.26405L7.58993 6.22077C7.6657 6.11641 7.7464 6.01587 7.8337 5.9166C8.03629 4.93534 7.86993 3.76318 7.41699 2.8061C7.19628 2.34283 6.92781 1.97884 6.67087 1.77139C6.61783 1.72827 6.55871 1.68986 6.49463 1.65684L6.48969 1.6543ZM21.5999 1.70521L21.5966 1.70648C21.5325 1.73949 21.4734 1.7779 21.4203 1.82102C21.1634 2.02847 20.8933 2.39374 20.6742 2.85701C20.1966 3.86754 20.0368 5.11734 20.2954 6.13041L20.3909 6.25387L20.4041 6.27168H20.4535C21.2709 6.27186 22.0841 6.36273 22.8681 6.5415C23.0097 5.11097 22.9307 3.94136 22.6573 3.07719C22.509 2.61265 22.3114 2.25883 22.0824 2.01829L22.0759 2.01066C21.9604 1.87654 21.7975 1.77095 21.6064 1.70648L21.5999 1.70521Z'

function Logo({ size = 13 }) {
  return jsx('svg', {
    width: size,
    height: size,
    viewBox: '0 0 28 28',
    fill: 'none',
    'aria-hidden': true,
    style: { flexShrink: 0, color: 'currentColor' },
    children: jsx('path', {
      fillRule: 'evenodd',
      clipRule: 'evenodd',
      d: OLLAMA_LOGO_PATH,
      fill: 'currentColor'
    })
  })
}

// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------

function pct(value) {
  const n = Number(value) || 0
  return Math.max(0, Math.min(1, n))
}

function colorFor(value) {
  const p = pct(value)
  if (p >= 0.9) return C.bad
  if (p >= 0.75) return C.warn
  return C.accent
}

// ---------------------------------------------------------------------------
// Presentational pieces (inline-styled)
// ---------------------------------------------------------------------------

function Bar({ value, height = 6 }) {
  const p = pct(value)
  return jsx('div', {
    style: {
      height,
      width: '100%',
      borderRadius: 999,
      overflow: 'hidden',
      background: C.track
    },
    children: jsx('div', {
      style: {
        height: '100%',
        width: `${(p * 100).toFixed(1)}%`,
        borderRadius: 999,
        background: colorFor(p),
        transition: 'width 0.4s ease'
      }
    })
  })
}

// Ring gauge — the "fancier" graph for the popover.
function Ring({ value, size = 66, stroke = 6 }) {
  const p = pct(value)
  const r = (size - stroke) / 2
  const c = 2 * Math.PI * r

  return jsxs('div', {
    style: { position: 'relative', width: size, height: size, flexShrink: 0 },
    children: [
      jsxs('svg', {
        width: size,
        height: size,
        viewBox: `0 0 ${size} ${size}`,
        children: [
          jsx('circle', {
            cx: size / 2,
            cy: size / 2,
            r,
            fill: 'none',
            stroke: C.track,
            strokeWidth: stroke
          }),
          jsx('circle', {
            cx: size / 2,
            cy: size / 2,
            r,
            fill: 'none',
            stroke: colorFor(p),
            strokeWidth: stroke,
            strokeLinecap: 'round',
            strokeDasharray: c,
            strokeDashoffset: c * (1 - p),
            transform: `rotate(-90 ${size / 2} ${size / 2})`,
            style: { transition: 'stroke-dashoffset 0.4s ease' }
          })
        ]
      }),
      jsx('div', {
        style: {
          position: 'absolute',
          inset: 0,
          display: 'flex',
          alignItems: 'center',
          justifyContent: 'center',
          fontSize: 14,
          fontWeight: 600,
          fontVariantNumeric: 'tabular-nums',
          color: C.primary
        },
        children: `${Math.round(p * 100)}%`
      })
    ]
  })
}

// Per-model horizontal bars (request count relative to the top model).
function ModelBars({ models }) {
  if (!models || models.length === 0) {
    return jsx('div', {
      style: { fontSize: 11, color: C.quaternary },
      children: 'No requests yet'
    })
  }

  const sorted = [...models].sort(
    (a, b) => (b.request_count || 0) - (a.request_count || 0)
  )
  const max = sorted[0].request_count || 1

  return jsxs('div', {
    style: { display: 'flex', flexDirection: 'column', gap: 7 },
    children: sorted.map(m => {
      const w = ((m.request_count || 0) / max) * 100
      return jsxs('div', {
        style: { display: 'flex', flexDirection: 'column', gap: 3 },
        children: [
          jsxs('div', {
            style: {
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'space-between',
              gap: 8,
              fontSize: 11
            },
            children: [
              jsx('span', {
                style: {
                  overflow: 'hidden',
                  textOverflow: 'ellipsis',
                  whiteSpace: 'nowrap',
                  color: C.secondary
                },
                children: m.name
              }),
              jsx('span', {
                style: {
                  flexShrink: 0,
                  fontVariantNumeric: 'tabular-nums',
                  color: C.tertiary
                },
                children: String(m.request_count || 0)
              })
            ]
          }),
          jsx('div', {
            style: {
              height: 5,
              width: '100%',
              borderRadius: 999,
              overflow: 'hidden',
              background: C.track
            },
            children: jsx('div', {
              style: {
                height: '100%',
                width: `${w}%`,
                borderRadius: 999,
                background: C.accent
              }
            })
          })
        ]
      })
    })
  })
}

// ---------------------------------------------------------------------------
// Shared body used by BOTH the popover and the pane.
// ---------------------------------------------------------------------------

function UsageContent({ compact }) {
  const client = useQueryClient()
  const { data, error, isFetching } = useQuery({
    queryKey: USAGE_KEY,
    queryFn: fetchUsage,
    refetchInterval: 60_000
  })

  const refresh = () => {
    haptic('tap')
    client.invalidateQueries({ queryKey: USAGE_KEY })
  }

  const header = jsxs('div', {
    style: {
      display: 'flex',
      alignItems: 'center',
      justifyContent: 'space-between',
      gap: 8
    },
    children: [
      jsxs('div', {
        style: { display: 'flex', alignItems: 'center', gap: 6, color: C.primary },
        children: [
          jsx(Logo, { size: 15 }),
          jsx('span', {
            style: { fontSize: 12, fontWeight: 600 },
            children: 'Ollama Cloud'
          })
        ]
      }),
      jsx('button', {
        type: 'button',
        onClick: refresh,
        title: 'Refresh',
        style: {
          display: 'inline-flex',
          alignItems: 'center',
          border: 'none',
          background: 'transparent',
          cursor: 'pointer',
          color: C.tertiary,
          padding: 2
        },
        children: jsx(Codicon, { name: 'refresh', size: '0.85rem', spinning: isFetching })
      })
    ]
  })

  let body
  if (error) {
    body = jsx('div', {
      style: { fontSize: 11, color: C.bad },
      children: `Failed to load: ${error.message || error}`
    })
  } else if (!data) {
    body = jsx('div', {
      style: { fontSize: 11, color: C.quaternary },
      children: 'Loading…'
    })
  } else {
    // Single ring gauge for the monthly usage credit pool.
    const rings = jsxs('div', {
      style: {
        display: 'flex',
        alignItems: 'flex-start',
        justifyContent: 'space-around',
        gap: 8
      },
      children: [
        jsxs('div', {
          style: { display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 },
          children: [
            jsx(Ring, { value: data.monthly?.usage, size: 84 }),
            jsx('span', { style: { fontSize: 11, color: C.secondary }, children: 'This month' }),
            jsx('span', {
              style: { fontSize: 10, color: C.quaternary },
              children: 'resets on plan anniversary'
            })
          ]
        })
      ]
    })

    // Per-model breakdown for the current month.
    const section = (key, title, models, max) =>
      jsxs(
        'div',
        {
          style: { display: 'flex', flexDirection: 'column', gap: 6 },
          children: [
            jsx('div', {
              style: { fontSize: 11, fontWeight: 500, color: C.secondary },
              children: title
            }),
            jsx('div', {
              style: { maxHeight: max, overflowY: 'auto', paddingRight: 2 },
              children: jsx(ModelBars, { models })
            })
          ]
        },
        key
      )

    const monthlyModels = data.monthly?.models || []
    const breakdown = jsxs('div', {
      style: { display: 'flex', flexDirection: 'column', gap: 12 },
      children: [
        section('monthly', 'This month', monthlyModels, compact ? 260 : 320)
      ]
    })

    body = jsxs('div', {
      style: { display: 'flex', flexDirection: 'column', gap: 12 },
      children: [rings, breakdown]
    })
  }

  return jsxs('div', {
    style: { display: 'flex', flexDirection: 'column', gap: 12, width: '100%', padding: 8 },
    children: [header, body]
  })
}

// ---------------------------------------------------------------------------
// Statusbar chip label — live text with the logo. Rendered as the `label`
// (ReactNode) of a declarative StatusbarItem so the item keeps its toggleLabel
// and shows up in the bar's right-click show/hide menu like native chips.
// ---------------------------------------------------------------------------

function ChipLabel() {
  const { data, error } = useQuery({
    queryKey: USAGE_KEY,
    queryFn: fetchUsage,
    refetchInterval: 60_000
  })

  // The logo stays neutral; the percentage is tinted by its own severity
  // (accent under 75%, yellow >=75%, red >=90%) — same scale as the gauge.
  let content
  if (error) {
    content = jsx('span', { style: { color: C.bad }, children: 'err' })
  } else if (!data) {
    content = jsx('span', { style: { color: C.tertiary }, children: '…' })
  } else {
    const mv = pct(data.monthly?.usage)
    content = jsxs('span', {
      style: { display: 'inline-flex', gap: 5, color: colorFor(mv) },
      children: [
        jsx('span', { style: { color: C.quaternary }, children: 'M:' }),
        `${Math.round(mv * 100)}%`
      ]
    })
  }

  return jsxs('span', {
    style: {
      display: 'inline-flex',
      alignItems: 'center',
      gap: 5,
      fontVariantNumeric: 'tabular-nums'
    },
    children: [jsx(Logo, { size: 13 }), content]
  })
}

// ---------------------------------------------------------------------------
// Plugin
// ---------------------------------------------------------------------------

export default {
  id: ID,
  name: 'Ollama Cloud Usage',
  description:
    'Live Ollama Cloud usage — monthly usage credit pool and per-model request counts, from the official /api/usage endpoint.',
  defaultEnabled: false,
  register(ctx) {
    rest = ctx.rest
    ctx.onDispose(() => {
      rest = null
    })

    ctx.registerMany([
      {
        // Declarative StatusbarItem (NOT a render escape-hatch): the app strips
        // everything but {id,render} from a render contribution, which drops
        // toggleLabel and makes the chip permanently visible. A data item keeps
        // toggleLabel, so it lands in the bar's show/hide menu like native chips.
        id: 'chip',
        area: 'statusBar.right',
        order: 90,
        data: {
          id: ID,
          variant: 'menu',
          // Live label + the popover, both native to the statusbar's own menu
          // chip (opens upward, portaled, collision-aware — no manual side/align).
          label: jsx(ChipLabel, {}),
          title: 'Ollama Cloud usage',
          toggleLabel: 'Ollama Cloud',
          menuAlign: 'end',
          menuClassName: 'w-72',
          menuContent: () => jsx(UsageContent, { compact: true })
        }
      },
      {
        id: 'refresh',
        area: PALETTE_AREA,
        data: {
          id: 'ollama-cloud-usage.refresh',
          label: 'Ollama Cloud: Refresh usage',
          keywords: ['ollama', 'usage', 'quota', 'cloud'],
          run: () => queryClient.invalidateQueries({ queryKey: USAGE_KEY })
        }
      }
    ])
  }
}
